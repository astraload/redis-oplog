...
export default class ObservableCollection {
     ...
     * @param {*} param
     */
    constructor({ multiplexer, matcher, sorter, cursorDescription, driver }) {
        this.driver = driver;
    ...
    add(doc, safe = false) {
        ...
        this._addMetricsToKadira(doc, '_addPublished');
    }
    ...
    change(doc, modifiedFields) {
        ...
        if (!_.isEmpty(changed)) {
            ...
            this._addMetricsToKadira(doc, '_changePublished');
        }
    }
    ...
    remove(docId) {
        ...
        if (doc != null) {
            ...
            this._addMetricsToKadira(doc, '_removePublished');
        }
    }
    ...
    _addInitMetricsToKadira() {
        const Kadira = global.Kadira;
        if (!Kadira) return;
        var coll = this.cursorDescription.collectionName;
        var query = this.cursorDescription.selector;
        var opts = this.cursorDescription.options;
        var docSize = Kadira.docSzCache.getSize(coll, query, opts, this.store);
        var count = this.store && this.store.size();

        const ownerInfo = this._getOwnerInfo();
        if (ownerInfo) {
            Kadira.models.pubsub.trackPolledDocuments(ownerInfo, count);
            Kadira.models.pubsub.trackDocSize(ownerInfo.name, "polledFetches", docSize*count);
        } else {
            const observeDriver = this.driver;

            if (!observeDriver._docSize) {
                observeDriver._docSize = {
                    polledFetches: 0
                };
            }

            observeDriver._polledDocuments = count;
            observeDriver._docSize.polledFetches = docSize*count
        }
    }

    _addMetricsToKadira(doc, fnName) {
        const Kadira = global.Kadira;
        if (!Kadira) return;
        const ownerInfo = this._getOwnerInfo();

        if (ownerInfo) {
            Kadira.models.pubsub.trackLiveUpdates(ownerInfo, fnName, 1);
            ...
            if(fnName === "_addPublished") {
                ...
                Kadira.models.pubsub.trackDocSize(ownerInfo.name, "liveFetches", docSize);
            }
        } else {
            const observeDriver = this.driver;

            // If there is no ownerInfo, that means this is the initial adds
            if (!observeDriver._liveUpdatesCounts) {
                observeDriver._liveUpdatesCounts = {
                    _initialAdds: 0
                };
            }

            observeDriver._liveUpdatesCounts._initialAdds++;
            ...
            if (fnName === "_addPublished") {
                ...
                if (!observeDriver._docSize.initialFetches) {
                    observeDriver._docSize.initialFetches = 0;
                }
                ...
                observeDriver._docSize.initialFetches += docSize;
            }
        }
    }

    _getOwnerInfo() {
        return this.driver && this.driver._ownerInfo;
    }
}
